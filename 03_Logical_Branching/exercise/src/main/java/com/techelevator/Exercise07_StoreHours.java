...
public class Exercise07_StoreHours {
    ...
    /*
    Shelia's Seashell Store has a new website. Shelia wants to display if the store is
    open when someone views the website.
    ...
    The store is open if the current hour is equal to or greater than the open
    time and less than the closing time.
    ...
    Her store is open between 8 am (hour 8) and 5 pm (hour 17) on Monday (day M), Wednesday (day W),
     and Friday (day F).
    Implement the logic to determine if the store is open based on the current hour
    and the current day.
    ...
    In the summer, the store is open until 8 pm (hour 20) on Wednesday (day W)
    and is also open Saturday (day S) from 9 am (hour 9) to 3 pm (hour 15).

    Implement the logic to determine if the store is open based on the current hour,
     the current day, and if it's summer.
    ...
    public boolean isStoreOpen(int currentHour, char currentDay, boolean isSummer) {
        if ((currentHour >= 8 && currentHour < 17) && (currentDay == 'M'||currentDay == 'W'||currentDay == 'F') && !isSummer) {
           return true;
        }
        else if(((currentHour>=9 && currentHour<15)) && (currentDay=='S') && isSummer){
            return true;
        }
        else if(((currentHour >= 8 && currentHour < 17) && (currentDay == 'M'||currentDay == 'F') ) && isSummer){
            return true;
        }
        else if(((currentHour >=8 && currentHour<20)) && ((currentDay=='W') && isSummer)){
            return true;
        }else {return false;}
    }
}
