...
public class AuctionServiceTest {
    ...
    private static final String EXPECTED_API_URL_NO_TRAIL_SLASH = "http://localhost:3000/auctions";
    ...
    public void getAllAuctions_returns_all_auctions() {
        // Arrange
        lenient().when(mockRestTemplate.getForObject(eq(EXPECTED_API_URL), eq(Auction[].class)))
                .thenReturn(expectedAuctions);
        lenient().when(mockRestTemplate.getForObject(eq(EXPECTED_API_URL_NO_TRAIL_SLASH), eq(Auction[].class)))
                .thenReturn(expectedAuctions);
    ...
    public void getAuctionsMatchingTitle_returns_correct_auctions() {
        // Arrange
        lenient().when(mockRestTemplate.getForObject(eq(EXPECTED_API_URL + "?title_like=Zero"), eq(Auction[].class)))
                .thenReturn(expectedAuctions);
        lenient().when(mockRestTemplate.getForObject(eq(EXPECTED_API_URL_NO_TRAIL_SLASH + "?title_like=Zero"), eq(Auction[].class)))
                .thenReturn(expectedAuctions);
    ...
    public void getAuctionsAtOrBelowPrice_returns_correct_auctions() {
        // Arrange
        lenient().when(mockRestTemplate.getForObject(eq(EXPECTED_API_URL + "?currentBid_lte=23.25"), eq(Auction[].class)))
                .thenReturn(expectedAuctions);
        lenient().when(mockRestTemplate.getForObject(eq(EXPECTED_API_URL_NO_TRAIL_SLASH + "?currentBid_lte=23.25"), eq(Auction[].class)))
                .thenReturn(expectedAuctions);
